...
var jA = LanguageMapping{
	...
	FlagCommitLogShowRefs:               "コミットログに各コミットを指すブランチ、タグ、HEAD の参照を表示するかを設定します: 'true' で表示、'false' で非表示。デフォルトは true です",
	FlagCommitLogShowAllBranches:        "コミットログにすべてのローカルブランチ、リモート追跡ブランチ、タグのコミットを表示するかを設定します: 'true' で表示、'false' で現在のブランチのみ。デフォルトは false です",
	...
	CommitLogShowRefsEnabled:            "コミットログの参照表示が有効になりました",
	CommitLogShowRefsDisabled:           "コミットログの参照表示が無効になりました",
	CommitLogShowRefsSetError:           "無効な入力です。コミットログの参照を表示するには `true`、非表示にするには `false` を入力してください",
	CommitLogShowRefsSaveError:          "コミットログの参照設定を保存できませんでした: %s",
	CommitLogShowAllBranchesEnabled:     "全ブランチのコミットログが有効になりました",
	CommitLogShowAllBranchesDisabled:    "全ブランチのコミットログが無効になりました",
	CommitLogShowAllBranchesSetError:    "無効な入力です。すべてのブランチのコミットを表示するには `true`、現在のブランチのみを表示するには `false` を入力してください",
	CommitLogShowAllBranchesSaveError:   "全ブランチのコミットログ設定を保存できませんでした: %s",
	Branches:                            "ブランチ",
	...
	GitPushPopUpWorkingDirectory:                                   "作業ディレクトリ",
	GitPushPopUpArgv:                                               "argv",
	GitPushPopUpExitStatus:                                         "終了ステータス",
	GitPushPopUpExitStatusNotStarted:                               "開始されませんでした",
	GitPushPopUpExitStatusCancelled:                                "キャンセルされました",
	GitPushPopUpCouldNotStart:                                      "プッシュを開始できませんでした: %s",
	GitPushPopUpNonZeroExit:                                        "プッシュは終了ステータス %d で失敗しました。原因は上記の出力を確認してください。",
	GitPushPopUpReadFailure:                                        "プッシュは終了ステータス 0 で終了しましたが、出力を完全に取得できませんでした: %s",
	GitPushPopUpStdout:                                             "stdout",
	GitPushPopUpStderr:                                             "stderr",
	GitPushPopUpStreamEmpty:                                        "（出力なし）",
	GitPushPopUpReconciling:                                        "リポジトリの状態を更新中",
	GitPushPopUpPushSucceeded:                                      "プッシュに成功しました。",
	GitPushPopUpRefreshSucceeded:                                   "プッシュに成功し、リポジトリの状態を確認しました。",
	GitPushPopUpRefreshFailed:                                      "プッシュ後の状態更新に失敗しました: %s。最後に正常だった状態を保持しています。",
